...
playHumanBtn.addEventListener("click", () => {
    ...
    // let server assign roles as before (reload board)
    chess.reset();
    socket.emit && socket.emit("requestNew"); // optional - server can handle resets; safe if no handler
...
socket.on("playerRole", (role) => {
    playerRole = role;
    // only set mode label for clarity
    modeLabel.textContent = vsComputer ? "Mode: Play vs Computer (Medium)" : `Mode: Online — you are ${role === 'w' ? 'White' : 'Black'}`;
    renderBoard();
});
